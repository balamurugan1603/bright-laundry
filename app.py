...
def order_page():
    ...
    if os.path.exists(today_excel_file):
        ...
        for cloth_type in summary_totals.keys():
            ...
            try:
                total_amount += df_today[df_today['Cloth Type'] == cloth_type].head(1)['Total Price'].values[0]
            except:
                pass
        ...
        # Generate WhatsApp message if there are order details
        if order_details:
    ...
    # Render the `order.html` template, including customer details
